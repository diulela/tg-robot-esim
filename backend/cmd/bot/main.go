...
func main() {
	...
	var productsHandler *botHandlers.ProductsHandler
	if esimService != nil {
		productsHandler = botHandlers.NewProductsHandler(
			telegramBot.GetAPI(),
			esimService,
		...
		// 注册 Inline 查询处理器
		inlineHandler := botHandlers.NewInlineHandler(
			telegramBot.GetAPI(),
			db.GetProductRepository(),
			appLogger,
		)
	...
	helpHandler := botHandlers.NewHelpHandler(telegramBot.GetAPI(), dialogService)
	...
	menuHandler := botHandlers.NewMenuHandler(telegramBot.GetAPI(), menuService)
